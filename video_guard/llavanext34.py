...
import torch
import sys
sys.path.append("/scratch/czr/video_guard/LLaVA-NeXT")
...
from video_guard.prompt_template import category_prompt, video_moderation
...
def parse_args():
    ...
    # parser.add_argument("--video_path", help="Path to the video folder.", required=True)
    # parser.add_argument("--output_dir", help="Directory to save the model results JSON.", required=True)
    # parser.add_argument("--output_name", help="Name of the file for storing results JSON.", required=True)
    parser.add_argument("--model-path", type=str, default="/scratch/czr/hf_models/LLaVA-NeXT-Video-34B-DPO")
...
def run_inference(args):
    ...
    for idx, item in tqdm(enumerate(data_list), total=len(data_list)):
        ...
        new_dict["video_name"] = video_path
        ...
        #     new_dict = {"label": label, "ground_truth": gt}
            # question = category_prompt[label].format(detail_instruction=detail_instruction)

        question = video_moderation
    ...
    # data_list[item]["predictions"] = prediction_list
        ...
        with open("/scratch/czr/video_guard/result/llava_next_34b_general.json", "w") as f:
